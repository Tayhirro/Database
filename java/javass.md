...
- **Groovy**：支持动态类型，用 `def` 来声明变量，编译器会自动推断类型。

- **Groovy**：自动为属性生成 getter 和 setter，可以直接通过属性访问
...
```
object.with{		//delegate直接设置为object
...
- `this` 关键字总是指向**定义当前代码片段的类实例**



## Traits   

- 类似interface，只不过将方法实现
...
object.with{
	example.delegate=it
	example.resolveStrategy=Closure.DELEGATE_FIRST
	example()
...
- AS  闭包转换为接口
- 在转换为接口的时候，groovy会自动生成一个匿名类，闭包自动代理这个匿名类，并且实现匿名类的接口（比如说接口内的configure()函数）
- 类似java的接口实例化，避免了浪费类

```
interface PizzaConfigurer{
	void configure()
}
def getPizza ={
	setSize "large"
...
- 使用bind创建部分应用 在**定义闭包**的时候直接传递上下文
- curry()   recurry()

```
def setPizzaConfig ={shop,size,address.payment ->
	shop.setSize(size)
	shop.setAddress(address)
	shop.setPayment(payment)
}

def setPizzaForPs1 = setPizzaConfig.curry(ps1)

setPizzaForPs1("large", "xxx street", "WeChat")
...
```
def newContextPizza =getPizza.rehyudrate(ps1,ths,ps1)
//owner delegate this 
newContextPizza.resolveStrategy = Closure.DELEGATE_FIRTST
newContextPizza()
...
-  **单参数闭包**
  - 对于只有一个参数的闭包，可以省略参数列表，默认参数名称为 `it`。
  - 或者显式定义
...
def singleParamClosure = { name -> println "Hello, $name!" }
```
...
```groovy
def varargsClosure = { String... words int s=20-> words.each { println it } }
...
- getOwner()
- `owner` 指向定义闭包的对象，通常是外部的类或者另一个闭包
...
- Closure.OWNER_FIRST是默认策略。优先在**owner**寻找，owner没有再**delegate**
- Closure.DELEGATE_FIRST：优先在**delegate**寻找，delegate没有再owner
...
class Machine implements Printer, Scanner {
    @Delegate Printer printer = new Printer() {
        void print() { println "Printing..." }
    }
    @Delegate Scanner scanner = new Scanner() {
        void scan() { println "Scanning..." }
    }
}

def machine = new Machine()
machine.print()  // 输出：Printing...
machine.scan()   // 输出：Scanning...
...
```
class Config {
    @Delegate Map<String, Object> settings = [:]
}
...
println config.theme   // 输出：dark
println config.fontSize // 输出：14
println config.size()   // 输出：2
...
![image-20241012191637600](C:\Users\Tayhirro\AppData\Roaming\Typora\typora-user-images\image-20241012191637600.png)
...
![、](C:\Users\Tayhirro\AppData\Roaming\Typora\typora-user-images\image-20241012214343728.png)
