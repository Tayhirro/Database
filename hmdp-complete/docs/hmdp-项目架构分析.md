...
| 搜索域    | 统一入口、垂直域召回、查询标准化、分页和分组结果       | `tb_shop`、`tb_blog`、`tb_user` 的 MySQL 关键词基线；未来可接可重建搜索索引       |
...
> [!note] 边界说明
> “发布博客”只是博客聚合的创建命令，不是编辑、删除、点赞或评论的外层模块。图片资产服务同时支撑发布、编辑和删除，因此放在博客生命周期中说明；底层存储能力再由共享基础设施章节统一描述。搜索是面向店铺、博客、用户等多类内容的横向读取能力，因此接口和实现独立于任一业务 Controller；当前三个 MySQL 垂直域均由 `SearchController` 对外提供。
...
- **状态：** ✅ 已实现异步两段式秒杀（受理段 + 落库段），含取消、支付模拟、结果轮询、售罄订阅与开始提醒。
- **现实触发：** 已登录用户在店铺详情页点击"限时抢购"，随后用返回的 orderId 轮询抢购结果。
- **调用：** `VoucherOrderController.seckillVoucher()` → `VoucherOrderServiceImpl.seckillVoucher()`（受理段）→ lua/seckill.lua 原子扣减 → XADD seckill:stream:orders → `SeckillOrderStreamConsumer` 消费 → `fulfillOrder()`（落库段，事务）。
- **输入：** 路径参数 voucherId（秒杀券 ID）；无需请求体。
- **正常流程：** 校验券 type=1 与活动时间窗 → Lua（KEYS=seckill:stock:{voucherId}、seckill:ordered:{voucherId}；ARGV=userId；0 成功/1 未预热/2 售罄/3 重复）→ RedisIdWorker 生成订单号（1 位符号 + 31 位秒级时间戳 + 32 位 Redis 序列）→ XADD 消息（voucherId/userId/orderId）并立即返回 orderId；消费端事务内 FOR UPDATE 锁券行、`stock > 0` 条件扣减、INSERT 订单，`UNIQUE(user_id, voucher_id)` 冲突视为重复投递。
- **示例：** `POST /voucher-order/seckill/300` → `{"success":true,"data":1735689600123456789}`；随后 `GET /voucher-order/1735689600123456789` 轮询，data 为 null 表示还在落库，有值即最终状态。
- **数据与边界：** 消息投递失败会回滚 Lua（库存加回、资格移除）；消费失败的消息留在 PEL，重试任务每 30 秒 XCLAIM 认领（最短闲置 30 秒），超过 max-retry（默认 5）次转入死信 List seckill:stream:orders:dead 并补偿 Redis 库存与资格；取消订单只改状态为 4 并恢复 DB/Redis 库存各 1，资格保留（不可重复抢同一券）；支付是模拟接口（status 1→2），未接真实支付渠道。

#### `GET /voucher-order/{id}` — `queryOrder()`

- **状态：** ✅ 已实现；抢购结果轮询入口。
- **现实触发：** 用户秒杀受理成功拿到 orderId 后，页面轮询确认订单是否创建。
- **调用：** `VoucherOrderController.queryOrder()` → `VoucherOrderServiceImpl.queryOrderById()`。
- **输入：** 路径参数 orderId。
- **正常流程：** 订单不存在（还在异步队列）返回 data=null 表示处理中；存在则校验主人或管理员身份后返回 `VoucherOrderDTO`（id/voucherId/status/createTime/payTime）。
- **示例：** `GET /voucher-order/1735689600123456789` → `{"success":true,"data":{"id":1735689600123456789,"status":1}}`。
- **数据与边界：** 他人订单返回失败；订单状态 1 未支付、2 已支付、3 已核销、4 已取消。

#### `GET /voucher-order/mine` — `queryMyOrders()`

- **状态：** ✅ 已实现；本人订单列表，voucherId 可选、lastId 游标（ID 倒序一页 10 条）。
- **示例：** `GET /voucher-order/mine?voucherId=300&lastId=999` 返回某券下早于 999 的本人订单。
- **数据与边界：** 只查本人；游标取上一页最后一条的 id。

#### `POST /voucher-order/{id}/pay` 与 `POST /voucher-order/{id}/cancel` — `payOrder()` / `cancelOrder()`

- **状态：** ✅ 已实现；支付为模拟接口（无真实支付渠道），取消只对本人未支付订单生效。
- **正常流程：** 两者都是条件更新状态机——pay `status 1→2` 记 payType/payTime；cancel `status 1→4` 并恢复 DB/Redis 库存各 1（事务提交后），触发到货通知。
- **数据与边界：** 条件更新影响 0 行即状态已变化，返回失败；取消后资格保留（UNIQUE(user_id, voucher_id)），不可重复抢同一券。

#### `POST/DELETE/GET /voucher-order/seckill/{id}/subscribe` — `subscribeArrival()` 等

- **状态：** ✅ 已实现；售罄订阅、取消订阅与本人订阅状态查询。
- **正常流程：** 订阅写 Redis Set `seckill:subscribe:{voucherId}`；补货（管理调库存 0→正或取消订单回补）事务提交后清空集合并逐个发到货短信；`SeckillReminderJob` 每 60 秒扫描活动开始前 10 分钟的券，按 `seckill:reminded:{id}` 标记提醒一次。
- **数据与边界：** 短信走 SmsSender 日志实现；订阅数据是可丢失的辅助数据，Redis 清空不影响交易。

#### `GET /voucher/{id}` / `PUT /voucher/{id}` / `POST /voucher/{id}/stock` — 券详情与管理

- **状态：** ✅ 已实现；管理写接口仅 `hmdp.admin.user-ids` 白名单用户可用。
- **正常流程：** 详情对秒杀券补充实时库存与时间窗；修改走字段白名单（title/subTitle/rules/status/beginTime/endTime，shopId/type/面值不可改），秒杀时间窗同步 tb_seckill_voucher；调库存走条件 UPDATE（结果不为负），事务提交后以数据库新值覆盖 `seckill:stock:{id}`，0→正触发到货通知。
- **数据与边界：** 影响行数 0 返回失败；修改店铺归属与面值会破坏订单语义，白名单直接不允许。
...
#### `GET /search/suggest` — `suggest()`

- **状态：** ✅ 已实现搜索输入联想与热词兜底。
- **现实触发：** 用户在统一搜索框输入时前端防抖请求联想；未输入时展示热词。
- **调用：** `SearchController.suggest()` → `MySqlSearchSuggestionService`。
- **输入：** `keyword` 可选，最长 64 字符（与主搜索一致）。
- **正常流程：** 空关键词返回 `search:hot:` ZSet 的 `ZREVRANGE 0..9` 热词；非空时串行 3 条前缀 SQL：店铺 `name LIKE 'kw%' LIMIT 4`、博客 `title LIKE 'kw%' LIMIT 3`、用户 `nick_name LIKE 'kw%' LIMIT 3`，共最多 10 条 `SearchSuggestionDTO`（text/scope/targetId）。
- **示例：** `GET /search/suggest?keyword=火锅` 返回店名/笔记标题/昵称的前缀命中；`GET /search/suggest` 返回热词。
- **数据与边界：** 每条 SQL 独立降级（失败只记日志返回部分结果）；统一搜索成功后按关键词 ZINCRBY 记录热词（fail-open），暂无防刷；用户敏感列不参与匹配。

### I. 店铺评价方法

#### `POST /shop-review` — `createReview()`

- **状态：** ✅ 已实现店铺评价链路（发布/列表/删除/统计）。
- **现实触发：** 用户在店铺详情页对店铺打 1~5 星并留下文字与图片评价。
- **调用：** `ShopReviewController.create()` → `ShopReviewServiceImpl.createReview()`（事务：INSERT tb_shop_review + UPDATE tb_shop_review_stat）。
- **输入：** `shopId`、`rating`（1~5）、`content`（≤500 字）、`images`（≤9 个 URL，可选）。
- **正常流程：** 校验店铺存在与字段边界 → 插入明细（UNIQUE(shop_id, user_id) 拦截一人一店一评）→ 同事务累加统计（评价数 +1、评分总和 +rating；统计行不存在时初始化，并发首评撞主键后重试 UPDATE）。
- **示例：** `POST /shop-review` body `{"shopId":100,"rating":5,"content":"服务很好"}` → data 为评价 ID。
- **数据与边界：** 重复评价返回 409 语义；统计表独立于 tb_shop，平均分 = 评分总和/评价数（存总和避免浮点漂移）。

#### `GET /shop-review/list/{shopId}` — `listByShop()`（游客可访问）

- **状态：** ✅ 已实现；复合游标分页，作者批量装配。
- **正常流程：** 游标 =（上一页最后一条的 createTime UTC 毫秒，id），条件"时间更早或同秒 id 更小"，`ORDER BY create_time DESC, id DESC`，一页默认 10、上限 20；整页作者一次批量查询。
- **数据与边界：** 返回 list/hasMore/lastTime/lastId；作者可能已注销（昵称头像为 null 不影响评价展示）。

#### `DELETE /shop-review/{id}` — `deleteReview()`

- **状态：** ✅ 已实现；仅限本人评价，物理删除并同事务回退统计（GREATEST 防负数），删除后可重新评价（唯一键行已消失）。

#### `GET /shop-review/stat/{shopId}` — `statOfShop()`（游客可访问）

- **状态：** ✅ 已实现；返回评价数与平均分（1 位小数），无评价全 0。
...
| GET | `/blog/hot?cursor=&limit=` | 按 `(liked,id)` 游标查询热榜 |
...
> [!warning] 实时热榜游标只近似稳定
> 当前热榜仍按实时 `(liked DESC,id DESC)` 做 keyset pagination。`liked` 在翻页期间变化会让博客跨越游标边界，因此一次浏览可能漏项或重现。普通作者列表的 `create_time` 不变，不存在这个问题。要获得会话级稳定热榜，需要在得到 Redis 新键授权后发布“版本化 ID List 快照”，游标携带 `snapshotVersion + offset`；Redis 只保存可重建排名，不承载点赞关系真相。
