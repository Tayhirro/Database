    ...
    public SmsSender missingSmsSender(@Value("${hmdp.sms.provider:log}") String provider) {
        return new SmsSender() {
            ...
            public void sendLoginCode(String phone, String code) {
                throw noProvider(provider);
            }

            @Override
            public void sendVoucherArrival(String phone, String voucherTitle) {
                throw noProvider(provider);
            }
        };
    }

    private static IllegalStateException noProvider(String provider) {
