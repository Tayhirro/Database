...
- 相关：
  - `SpringApplication`（创建并驱动 refresh）：见 [springboot/modules/SpringApplication.md](SpringApplication.md)
  - 外部化配置（`Environment`）：见 [springboot/modules/ExternalizedConfiguration.md](ExternalizedConfiguration.md)
  - 自动配置：见 [springboot/modules/AutoConfiguration.md](AutoConfiguration.md)
- 特例：Web 应用会使用 Web 相关的 `ApplicationContext` 实现并绑定 WebServer（见 [springboot/modules/EmbeddedWebServer.md](EmbeddedWebServer.md)）。
