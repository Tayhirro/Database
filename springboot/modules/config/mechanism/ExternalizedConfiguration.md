...
- 配置视图：`Environment`（包含 profiles 与 `PropertySource` 序列，见 [../../core/context/interface/Environment.md](../../core/context/interface/Environment.md)）
...
- 属性绑定：`@ConfigurationProperties`（见 [springboot/modules/config/mechanism/ConfigurationProperties.md](ConfigurationProperties.md)）

## 关系：上级/下级/等价/特例/推广
- 上级：配置管理（configuration management）。
- 下级：`Environment`（见 [../../core/context/interface/Environment.md](../../core/context/interface/Environment.md)）、profiles、属性绑定。
- 相关：自动配置条件（见 [springboot/modules/config/mechanism/AutoConfiguration.md](AutoConfiguration.md)）。
