...
- 关联：候选列表的聚合机制见 [springboot/modules/SpringFactories.md](SpringFactories.md)

## 关系：上级/下级/等价/特例/推广
- 上级：配置类导入（Spring 配置类解析阶段的一类输入）。
- 相关：外部化配置（条件判断输入）、配置属性绑定、starter 依赖组织（见 [springboot/modules/Starters.md](Starters.md)）。
