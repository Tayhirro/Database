...
- **没有图**：主文方法里不存在物体间交互项、图神经网络或任何 relational 模块；
- **连显式转移模型都没有**：$f_z$ 只是概念假设，没有网络去学它；"动力学重放"是把推断出的 $z^t$ 序列换视角重新渲染，时间推进靠迭代摊销推断，不是学到的 rollout；
- 时间查询限于观测区间内（支持不等间隔），主文没有展示超出观测序列的外推；
- 场景是合成的（DRoom 房间、MJC-Arm 机械臂）。[原文第 3.2 节、图 3](https://arxiv.org/pdf/2111.05393#page=9)
