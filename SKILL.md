...
复杂研究任务默认使用三阶段流程，而不是无限递归。该流程吸收 problem framing、Double Diamond 和复杂问题求解的思想：先发散建立问题空间，再收敛定义真正要回答的单元，再聚焦调研并交付。子 agent 不能创建子子 agent；每轮都由主 agent 创建任务、读取结果、做 checkpoint。
...
- Round 1 是探索和建模，不是最终回答；如果已经派发子 agent，必须先进入 checkpoint。
...
- 每个 answer unit 必须标记 promoted / resolved / deferred / dropped，并写明理由。
- promoted：需要进入下一轮聚焦调研。
- resolved：已有直接证据足够回答，不需要再开子 agent。
...
- 不允许用 Round 1 的背景材料直接替代最终答案，除非所有 answer units 都被标记为 resolved。

Checkpoint 2：Answer Unit Resolution
- 发生在 Round 2 后。
- 目标：判断每个 promoted answer unit 是否已经有足够证据、边界和结论强度。
- 如果仍缺关键证据，进入 gap-fill；如果关键 claim 需要核查，进入 verification；如果证据已足够，标记 resolved。
...
Round 3：Synthesis / Verification / Deliver
- 目标：综合、证据分级、必要时补查或 verification，然后输出最终笔记。
- 区分文献直接支持、综合推断和不确定项。
- 如果存在 promoted 但未完成的 answer unit，不允许最终输出。
- 如果仍有关键 claim 缺少证据，进入 gap-fill 或 verification；否则写最终笔记。
```
...
- Checkpoint：抽取 I1/I2/I3 等创新点作为 answer units。
- Round 2 Focused Investigation：
...
2. 在 `research.md` 的“Subtasks / 子任务清单”小节列出每一轮、每个子任务、负责方向、状态和输出文件路径。
3. 在 `research.md` 的“Round 1：Retrieval 子任务”小节列出每个 retrieval 子 agent 的任务和范围。
4. 如果输入是论文、综述或文章，Round 1 的第一个子任务必须是“原文结构理解”，用于概括文章主线、章节 / taxonomy 和作者 claim。
5. 如果进入 Round 2 Focused Investigation、gap-fill 或 verification，继续在同一个 `research.md` 里追加“Round 2：Focused / Gap-Fill 子任务”或“Round 3：Verification / Synthesis 子任务”清单。
6. 在 `research.md` 的“Follow-up Candidates / 后续候选问题状态”小节记录每个候选问题的状态：promoted / deferred / dropped / resolved。
...
- 轮次：round1-retrieval / round2-focused / round2-gapfill / round3-verification
- 角色：retrieval / focused-investigation / gap-fill / verification
- 独立问题：
- 为什么需要单独查证：
- 需要查：
- 不需要查：
...
第 3.1 节决定当前处于 Round 1 Orientation、Round 2 Focused Investigation，还是 Round 3 的 gap-fill / verification。本节只负责每一轮如何开启一个或多个子 agent。不要把子 agent 启动协议单独理解成新的研究流程；它是三阶段流程里的执行层。
...
- 拆分决策：
  ...
  - 如果输入是论文、综述或文章，Round 1 的第一个 retrieval 子任务必须是“原文结构理解”：先概括原文本身讲什么、章节 / taxonomy 如何组织、作者 claim 是什么、哪些内容只是背景、哪些 claim 需要外部查证。
  - 原文结构理解不做外部概念扩展，不把任务改写成“查某个相关领域是什么”；外部背景知识只能作为后续 retrieval 子任务或 gap。
...
第 2 轮：Focused Investigation / Gap-Fill
- 目标：只针对 checkpoint 提升的 answer units 或证据缺口补查，不重复覆盖已有结论。
- 启动方式：主 agent 根据 promoted answer units 或 gap list 创建一个或多个 focused / gap-fill 任务。
- 输出：每个 gap-fill 子 agent 必须说明 gap 是否被填补；如果无法填补，要明确写“未找到直接证据”，不能用弱相关材料硬补。

第 3 轮：Verification
- 目标：检查关键 claim 是否真的被引用来源支持。
- 启动方式：主 agent 抽取最重要的 5-10 条 claim，派 verification 子 agent 逐条核查。
- 输出：对每条 claim 给出 SUPPORTED / PARTIAL / UNSUPPORTED，并说明是否需要改写、降级为综合推断或删除。
```
...
- 区分“文献直接支持”和“基于文献的综合推断”；
- 不要编造论文或概念；
- 明确自己属于哪一轮：`round1-retrieval` / `round2-focused` / `round2-gapfill` / `round3-verification`；
...
### round2-focused / Focused Investigation

- 目标：只研究一个 promoted answer unit。
- 重点：回答该单元的对象、机制 / 结构 / 判定标准、直接证据、综合推断、边界和反例。
- 禁止：不要泛泛补背景；不要覆盖其他 answer units；不要把相邻概念支持写成文献直接支持。

### round2-gapfill / Gap-Fill

- 目标：只补一个明确证据缺口。
- 重点：说明 gap 是否被填补；如果找不到直接证据，必须写“未找到直接证据”并降级结论。
- 禁止：不要重复 Round 1 已经覆盖的背景；不要用弱相关材料硬补。

### round3-verification / Verification

- 目标：核查关键 claim 是否被引用来源支持。
- 重点：逐条输出 SUPPORTED / PARTIAL / UNSUPPORTED，并说明保留、改写、降级或删除建议。
- 禁止：不要新增大范围调研；不要引入未核查的新 claim。
...
- `rounds/round2-focused/`：保存第二轮围绕 promoted answer units 的聚焦调研结果。
- `rounds/round2-gapfill/`：保存第二轮针对证据缺口的补查结果。
- `rounds/round3-verification/`：保存第三轮关键 claim 的引用核查结果。
...
子 agent 每一轮返回调研结果后，主 agent 都必须执行本节作为 checkpoint。若当前不是最终轮，执行到 answer units / gap / verification 状态更新后，决定进入下一轮或停止；不要直接写最终笔记。

执行要求：

1. 逐个读取 `tmp/*问题研究/` 下的 `research.md` 和 `rounds/`；
2. 先合并 `rounds/round1-retrieval/`：去重来源，按领域整理证据，记录初步共识和冲突；
3. 如果输入是论文、综述或文章，先读取 Round 1 中的“原文结构理解”子任务，并把文章主线、章节 / taxonomy、作者 claim、证据组织方式和原文边界作为后续综合的骨架；外部 retrieval 只能补充或校验原文理解，不能替代“文章讲了什么”；
4. 收集所有子 agent 输出中的 `Follow-up Candidates / 后续候选问题`，追加或更新到 `tmp/followup_candidates.md`；
5. 建立或更新 answer units 状态表：合并重复候选，标记来源，判断它是否是最终答案必须单独回应的对象；
6. 只把 promoted answer units 提升为下一轮任务；提升时创建或更新对应 `research.md`，记录 `parent_issue`、`derived_from`、`source_subagent_output`、`promotion_reason` 和输出目录；
...
8. 执行 pre-final gate：
   ...
   - 每个 answer unit 必须有 promoted / deferred / dropped / resolved 状态和理由；
...
11. 抽取最重要的 5-10 条 claim，派 `rounds/round3-verification/` 检查引用是否真的支持 claim；
12. 根据 verification 结果处理 claim：
...
- **通用分析骨架**
	...
	- **依据与可信度**：
		- 它凭什么成立：论文原文、实验、ablation、benchmark、理论证明、综述、经典理论、复现结果或外部工具。
