...
> [!tip] 这些线不是孤立的
> 它们之间有很多交叉。理解这些交叉，比死记每条线的细节更重要。

| 交叉点 | 说明 |
|--------|------|
| 白盒 → 黑盒 | 迁移攻击就是"在白盒模型上生成，在黑盒模型上用" |
| 白盒 → 架构特化 | ViT 特化攻击的底层仍然是迭代梯度优化，只是改了梯度计算和采样方式 |
| 黑盒 → 自动化评估 | AutoAttack 里的 Square Attack 就是黑盒攻击 |
| 物理世界 → 超越 Lp | 对抗补丁本质上不是 Lp 约束，而是空间约束 |
| GAN 生成 → L2L | AdvGAN 可以看成 L2L 的早期形态——让网络学习生成扰动 |
| 所有线 → L2L | L2L 的终极目标是用一个 learned optimizer 统一所有攻击策略的设计 |
