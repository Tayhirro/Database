...
对抗性攻击的发展，本质上是一个"攻击策略从"人工设计走向自动学习"的过程：从最早手写梯度公式（FGSM），到精心设计优化目标（C\&W），到自动组合评估（AutoAttack），最终走向**让神经网络自己学怎么攻击**（L2L / Meta-Learning）。每一步都是在回答同一个问题——怎么用最小的扰动把模型骗过去。
...
前两个阶段暴露了一个比"防御不够强"更严重的问题：**很多防御根本就是假的，只是当时没人能证明它是假的。**

这件事反复发生了很多次，典型的剧情是这样的：
...
5. 原来那个防御根本没用，只是 FGSM/PGD 碰巧打不动它而已
```

最典型的例子是 **Defensive Distillation**（Papernot et al., 2016）——用 PGD 攻击，看起来极其鲁棒；C&W 一出来，直接打穿。类似的"假鲁棒"防御在 2017–2019 年间层出不穷。

所以研究社区意识到：**在改进防御之前，得先解决一个前置问题——怎么判断一个防御到底是真的有效，还是只是恰好骗过了你用的那种攻击？** 如果评估手段本身不可靠，你都分不清现有防御是真强还是假强，改进防御就无从谈起。

这就是为什么这个阶段的核心诉求不是"提升防御"，而是"让攻击评估变得可靠"**。

### 什么是梯度遮蔽（Gradient Masking / Obfuscation）

很多"假防御"之所以能骗过基于梯度的攻击，靠的就是这个机制：

**正常的鲁棒模型**：对抗样本真的不存在（或很难找到），模型的决策边界确实远离了正常数据点。PGD 沿梯度走，走遍了也找不到对抗样本——因为真的没有。

**梯度遮蔽的模型**：对抗样本仍然大量存在，但模型的梯度信号坏了——梯度为零、梯度噪声极大、或者梯度指向错误方向。PGD 沿梯度走，走不到对抗样本——不是因为没有，而是因为==梯度是瞎指的，PGD 被骗了==。
...
常见的梯度遮蔽手法包括：输入加随机噪声、用不可微的预处理（JPEG 压缩、量化）、在网络中插入不可微组件（随机 dropout、离散化操作）。这些操作让基于梯度的攻击"看不见路"，但对抗样本本身并没有消失。

> [!tip] 理解这个阶段的关键
> 逻辑链是：假防御泛滥 → 根因是评估不可靠（只用梯度攻击测，容易被梯度遮蔽骗） → 解决方案是标准化评估（AutoAttack，混入不依赖梯度的攻击） → 有了可靠评估之后，才能有意义地去改进防御。
>
> 你说的"提升防御的泛化性"当然是最终目标，但那是有了可靠评估**之后**的事。这个阶段解决的是前置问题。

### AutoAttack

> Croce \& Hein, "Reliable Evaluation of Adversarial Robustness with an Ensemble of Attacks", ICML 2020

**做了什么**：把四种互补的攻击组合在一起（APGD-CE、APGD-DLR、FAB、Square Attack），自动依次尝试，任何一种攻破就算攻破。关键设计：
- APGD：带**自适应步长**的 PGD，不需要手动调参
- Square Attack：**不依赖梯度**的黑盒攻击——这是专门用来检测梯度遮蔽的

**优点**：
- ==无参数==——不需要手动调步长、步数等超参，拿来就用
- 混合了白盒 + 黑盒攻击，**能有效检测梯度遮蔽类的虚假防御**（这是它最大的贡献）
...
> [!warning] 梯度遮蔽问题在后续 L2L 方法中同样存在
> 后面会讲到 L2L（学习化攻击）方法，那里梯度遮蔽的问题更加隐蔽——learned optimizer 的更新规则是黑盒的，你很难判断它到底是"真的学会了更强的攻击"，还是"学会了利用目标模型的梯度遮蔽走捷径"。
