  ...
  - `V`（把什么当作“可交互的实体”）
    ...
    - map-only（纯地图图）：节点=map 元素（lane/segment/polyline 等），专注“可行驶结构先验”（很多 map encoder 属于这类）。
      ...
      - lane graph vs polyline token（关键差异）：
        ...
        - 形状直觉：两者的几何输入常是“每条线一串点”`P ∈ R^{N×L×C}`，但做图消息传递时通常更新的是“每条线一个向量”`X ∈ R^{N×D}`（先对点序列编码/池化到 `D` 再在图上迭代）。
        - tensor 例子（同样 `N=4`，但 `E` 来源不同）：
          - lane graph：`X_lane ∈ R^{4×D}`，`edge_index_topo = [[0,1,2,3],[1,2,3,2]]`（successor/merge 等），可配 `edge_type`（succ/left/right/merge...）。
          - polyline token：点序列 `P ∈ R^{4×L×C}` → polyline encoder 得 `X_poly ∈ R^{4×D}`；按欧氏距离取 `k=2` 得 `N(0)={1,3}`、`N(2)={1,3}`（并行/相邻车道会互为近邻，即使拓扑边未显式标 left/right）。
