...
- 常见实现路线（本质都是在做条件依赖建模）：
  ...
  - graph：只在邻居上消息传递（稀疏、更可控）。
    ...
    - 2) 节点是什么（V 的语义）：  
      ...
      - 显式道路拓扑 lane graph（你说的“lane 节点 + 连通边”）：节点=lane / lane segment；边=拓扑连通（predecessor/successor/left/right/merge/split 等）。它的优势是“可行驶结构先验很强、可控”，缺点是对 HD map 质量/拓扑定义敏感（LaneGCN, ECCV 2020）。  
