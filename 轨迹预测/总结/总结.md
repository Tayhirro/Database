...
- `(X, 可选 I)` →（可选：Semantic Map CNN 得到 `v_n` 并拼到轨迹 token）→ Time Encoder → AgentFormer Encoder 得到历史特征 `C` → Prior `p(z|C)` / Posterior `q(z|Y,C)` → 采样 `Z` → 自回归 future decoder 生成 `Y_hat`（可采样 K 条）。
