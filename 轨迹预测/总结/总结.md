...
- 常见实现路线（本质都是在做条件依赖建模）：
  ...
  - graph：只在邻居上消息传递（稀疏、更可控），差别在“节点/边怎么定义”：
    - agent-agent 图：节点=车辆/行人，边=近邻/可见/同车道/潜在冲突等规则（可做动态建图）；消息传递更像“显式选择谁影响谁”。
    - agent-map 图：节点既包含 agent 也包含 map 元素（lane segment / crosswalk / polygon 等），边=投影到最近车道/可达关系，让交互直接受道路约束。
    - map 本身的图（显式道路拓扑）：节点=lane / lane segment，边=连通关系（left/right/prev/next/merge/split 等）；这是“显式编码：lane 节点 + 连通边”的典型做法。
    - 向量地图建模（VectorNet 一类）：把地图/轨迹拆成 polyline，先做 polyline 内部聚合，再做 polyline-graph 交互（更强调几何与结构先验，不一定是严格的车道拓扑图）。
