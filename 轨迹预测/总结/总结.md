...
- 交互建模可以拆成 4 个正交要素：`V`（token/节点语义）、`E`（邻接/掩码）、`φ`（聚合算子）、`schedule`（组织方式）。
  - `V`（把什么当作“可交互的实体”）
    - 纯 agent（social graph）：节点=agent，专注多车交互（例如 HiVT）。
    - agent + map（异构图）：节点=agent 与 map 元素，让道路几何/规则直接参与交互（例如 LaneGCN 的 actor-lane 消息流，MTR等mapfeature参与模型）。
    - lane graph（拓扑先验）：节点=lane/lane segment，边=拓扑连通（pre/succ/left/right/merge...），强可行驶结构但依赖 HD map 质量（LaneGCN 等）。
    - polyline token（VectorNet 范式）：节点=polyline（车道/边界/历史片段…），强调几何局部结构与可扩展性（VectorNet/TNT/MTR 等）。
