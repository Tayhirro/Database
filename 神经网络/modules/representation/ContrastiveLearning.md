...
### 7.1 Metric Learning（更大的范畴）
- 对比学习和度量学习非常接近，但 **Metric Learning** 更像“学一个任务相关距离/相似度”的大框架。
- Siamese、Triplet、Center Loss、ArcFace、Prototypical Networks 都可放到这个大框架下理解。
- 如果你是在找“度量学习总览”，看：[MetricLearning.md](MetricLearning.md)

### 7.2 Triplet loss（度量学习常见）
- 用距离 `d(·,·)` 与 margin `m`：
  - `L = max(0, d(a,p) - d(a,n) + m)`
- 更像“排序/间隔约束”，而 InfoNCE 更像“带温度的多类分类”（softmax over candidates）。

### 7.3 非对比（no-negatives）的自监督（相关但不同路子）
- BYOL / SimSiam 等方法不显式用负样本，通常用 stop-grad、predictor、动量分支等机制避免塌缩（representation collapse）。

### 7.4 原型学习（Prototypical Networks，few-shot）
- 用 support 样本计算每类原型（类中心），query 按距离原型最近来分类；属于度量/表示学习的近亲（见 `modules/representation/PrototypicalLearning.md`）。
