...
- 线性层：$y = Wx + b$，其中 $W \in \mathbb{R}^{fan\_out \times fan\_in}$
- 常见假设：$x_i$ 独立同分布、$E[x_i]=0$、$Var(x_i)=v$；$W_{ji}$ 独立同分布、$E[W_{ji}]=0$、$Var(W_{ji})=\sigma^2$，且与 $x$ 独立

**前向尺度（只看线性部分）**
- $y_j = \sum_i W_{ji} x_i$
- 目标：$Var(y_j) \approx Var(x_i) = v$

**推导**：
$$
\begin{aligned}
Var(W_{ji} \cdot x_i) &= Var(W_{ji}) \cdot Var(x_i) = \sigma^2 v \quad \text{（独立变量乘积）} \\
Var(y_j) &= \sum_{i=1}^{fan\_in} Var(W_{ji} \cdot x_i) = fan\_in \cdot \sigma^2 v \quad \text{（独立变量求和）} \\
\text{令 } Var(y_j) &= v \Rightarrow \sigma^2 = \frac{1}{fan\_in}
\end{aligned}
$$

**反向尺度（梯度传播）**
- $\frac{\partial L}{\partial x_i} = \sum_j W_{ji} \cdot \frac{\partial L}{\partial y_j}$

$$
\begin{aligned}
Var(dL/dx_i) &= fan\_out \cdot \sigma^2 \cdot Var(dL/dy_j) \\
\text{令 } Var(dL/dx_i) &= Var(dL/dy_j) \Rightarrow \sigma^2 = \frac{1}{fan\_out}
\end{aligned}
$$

**矛盾**：前向要求 $\sigma^2 = 1/fan\_in$，反向要求 $\sigma^2 = 1/fan\_out$

**Xavier/Glorot 初始化（兼顾前向与反向的折中）**
- 折中：取两者的调和平均

$$
\sigma^2 = \frac{2}{fan\_in + fan\_out}
$$

- 正态分布（`xavier_normal_`）：$W \sim N(0, std^2)$，$std = gain \cdot \sqrt{2/(fan\_in + fan\_out)}$
- 均匀分布（`xavier_uniform_`）：$W \sim U(-a, a)$，$a = gain \cdot \sqrt{6/(fan\_in + fan\_out)}$

**He/Kaiming 初始化（ReLU 家族常用）**
- 问题：ReLU 会把负值置零，约砍掉一半信号

**推导**：设 $a = ReLU(x)$，由于 $E[x]=0$，约一半样本为正：

$$
Var(a) \approx \frac{1}{2} Var(x)
$$

补偿效应：

$$
fan\_in \cdot \sigma^2 \cdot \frac{1}{2} v = v \Rightarrow \sigma^2 = \frac{2}{fan\_in}
$$
