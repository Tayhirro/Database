...
- “算不动”的不是 `p(z)`：`p(z)` 往往直接设成 `N(0,I)`；难的是 **`p(x|z)` 由神经网络参数化** 后，`p(x)` 变成一个连续混合分布，`∫ p(z)p(x|z)dz` 一般没有闭式解，进而 `p(z|x)=p(z)p(x|z)/p(x)` 也算不出来。
...
- `p(x)` 是否能超过 1 取决于 `x` 是离散还是连续：
  - 离散 `x`：`p(x)` 是概率，必然 `<= 1`。
  - 连续 `x`：`p(x)` 是概率密度 `p(x)`（density），数值可以 `> 1`（例如方差很小的高斯在均值附近密度会很大），但对任意区域的概率 `P(x∈S)=∫_S p(x)dx` 仍然 `<= 1`。
