---
title: "概念图（依赖关系 & 学习路线）"
date: "2026-02-03"
categories:
  - 文学
tags:
  - "literature/concept-map"
  - moc
description: 用来回答"先学什么 → 再学什么"以及"这个概念和那个概念什么关系"。
type: "concept-map"
---
# 概念图（依赖关系 & 学习路线）

> 用来回答"先学什么 → 再学什么"以及"这个概念和那个概念什么关系"。

---

## 1. 体裁继承关系（Genre Hierarchy）

```mermaid
graph TD
    subgraph 文学作品
        LW[Literary Work]
    end
    
    subgraph 诗歌
        P[Poetry 诗歌]
        LP[Lyric Poetry 抒情诗]
        NP[Narrative Poetry 叙事诗]
        DP[Dramatic Poetry 戏剧诗]
    end
    
    subgraph 散文
        PR[Prose 散文]
        F[Fiction 虚构]
        NF[Non-Fiction 非虚构]
        N[Novel 长篇]
        NV[Novella 中篇]
        SS[Short Story 短篇]
        E[Essay 散文]
        B[Biography 传记]
    end
    
    subgraph 戏剧
        D[Drama 戏剧]
        T[Tragedy 悲剧]
        C[Comedy 喜剧]
        TC[Tragicomedy 悲喜剧]
    end
    
    LW --> P
    LW --> PR
    LW --> D
    
    P --> LP
    P --> NP
    P --> DP
    
    PR --> F
    PR --> NF
    F --> N
    F --> NV
    F --> SS
    NF --> E
    NF --> B
    
    D --> T
    D --> C
    D --> TC
```

---

## 2. 流派演化（Movement Evolution）

```mermaid
graph LR
    subgraph 西方
        CL[Classicism 古典主义] --> ROM[Romanticism 浪漫主义]
        ROM --> REA[Realism 现实主义]
        REA --> NAT[Naturalism 自然主义]
        REA --> MOD[Modernism 现代主义]
        MOD --> PM[Postmodernism 后现代主义]
    end
```

### 流派特征对比
| 流派 | 核心价值 | 形式特征 | 代表作家 |
| --- | --- | --- | --- |
| 古典主义 | 理性、秩序、典范 | 遵循三一律、格律严整 | 拉辛、莫里哀 |
| 浪漫主义 | 情感、个性、自然 | 抒情强烈、形式自由 | 拜伦、雨果、济慈 |
| 现实主义 | 客观、真实、社会 | 细节真实、典型环境 | 巴尔扎克、托尔斯泰、狄更斯 |
| 现代主义 | 主观、实验、碎片 | 意识流、多视角、时空跳跃 | 乔伊斯、伍尔夫、卡夫卡 |
| 后现代主义 | 解构、戏仿、自反 | 元小说、互文、不确定性 | 博尔赫斯、卡尔维诺、品钦 |

---

## 3. 分析工具依赖图（Module Dependencies）

```mermaid
graph TD
    subgraph 基础层
        CHAR[Characterization 人物塑造]
        SET[Setting 背景设定]
        PLOT[Plot 情节]
    end
    
    subgraph 叙事层
        POV[Narrative POV 叙事视角]
        TIME[Narrative Time 叙事时间]
        STRUCT[Narrative Structure 叙事结构]
        UN[Unreliable Narrator 不可靠叙述]
    end
    
    subgraph 语言层
        META[Metaphor 比喻]
        SYM[Symbolism 象征]
        IRO[Irony 反讽]
        IMG[Imagery 意象]
    end
    
    subgraph 意义层
        THEME[Theme 主题]
        MOTIF[Motif 母题]
        ARCH[Archetype 原型]
    end
    
    CHAR --> POV
    PLOT --> STRUCT
    PLOT --> TIME
    POV --> UN
    
    META --> SYM
    IMG --> META
    
    CHAR --> ARCH
    MOTIF --> THEME
    SYM --> THEME
```

---

## 4. 学习路线建议

### 4.1 入门路线（从体裁入手）
1. **体裁基础**：Fiction → Poetry → Drama（了解三大类的基本特征）
2. **分析工具**：Narrative POV → Characterization → Theme（掌握最常用的分析视角）
3. **修辞基础**：Metaphor → Symbolism → Irony（理解语言层面的技巧）

### 4.2 进阶路线（从流派入手）
1. **流派演化**：Classicism → Romanticism → Realism → Modernism
2. **形式深入**：Sonnet → Stream of Consciousness → Free Verse
3. **主题分析**：Identity → Coming-of-Age → Mortality

### 4.3 实战路线（从作品入手）
1. 选一部作品 → 先看体裁归属
2. 识别使用了哪些 modules（叙事/修辞/主题）
3. 定位流派与文学史位置
4. 与同类作品对比

---

## 5. 文化地理层级（Cultural Geography Hierarchy）

```mermaid
graph TD
    subgraph 地理层级
        R[长江]
        YR[扬子江]
        XY[浔阳江]
        PP[湓浦口]
    end
    
    subgraph 文化意象
        FAREWELL[离别]
        EXILE[贬谪]
        RIVER[江湖]
    end
    
    R --> YR
    YR --> XY
    XY --> PP
    
    XY -.-> FAREWELL
    XY -.-> EXILE
    XY -.-> RIVER
```

### 文化地理空间的组织
| 层级 | 说明 | 例子 |
| --- | --- | --- |
| 一级空间 | 大型地理单元 | 长江、黄河、江南、中原 |
| 二级空间 | 具体地点 | 浔阳江、赤壁、长安、扬州 |
| 三级空间 | 具体点位 | 湓浦口、浔阳楼、黄鹤楼 |

---

## 6. 跨领域联系

### 与数学的类比
| 文学概念 | 数学类比 | 说明 |
| --- | --- | --- |
| 体裁（Genre） | 代数结构（Group/Ring/Field） | 定义明确的类别系统 |
| 流派（Movement） | 结构演化链 | 随时间演化的特征变迁 |
| 修辞手法（Rhetoric） | 运算/变换 | 可复用的表达操作 |
| 作品分析 | 实例验证 | 看作品是否符合某体裁/流派的定义 |

### 与文化地理的关联
| 文学元素 | 地理维度 | 说明 |
| --- | --- | --- |
| Setting 背景设定 | 文化地理空间 | 作品发生的具体空间 |
| 意象（Imagery） | 空间意象 | 地理空间承载的情感与象征 |
| 主题（Theme） | 空间主题 | 特定地理空间衍生的主题（如贬谪、离别） |
| 作家经历 | 空间轨迹 | 作家的贬谪、游历与空间书写 |

---

## 7. 常见问题导航

| 问题 | 去哪里找 |
| --- | --- |
| 什么是意识流？ | [structures/forms/StreamOfConsciousness.md](structures/forms/StreamOfConsciousness.md) |
| 怎么分析小说的叙事视角？ | [modules/NarrativePOV.md](modules/NarrativePOV.md) |
| 浪漫主义和现实主义有什么区别？ | [structures/periods/Romanticism.md](structures/periods/Romanticism.md) + [structures/periods/Realism.md](structures/periods/Realism.md) |
| 什么是不可靠叙述者？ | [modules/UnreliableNarrator.md](modules/UnreliableNarrator.md) |
| 怎么识别作品中的象征？ | [modules/Symbolism.md](modules/Symbolism.md) |
| 浔阳江在哪里？有何文学意义？ | [cultural-geography/浔阳江.md](cultural-geography/浔阳江.md) |
| 如何分析文学中的地理空间？ | [cultural-geography/README.md](cultural-geography/README.md) |
