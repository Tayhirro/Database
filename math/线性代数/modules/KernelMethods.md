---
title: 核方法（Kernel Methods）与“核函数判定链路”
date: "2026-01-11"
categories:
  - math
description: 核方法把“输入空间里看起来很非线性的几何/决策边界”，转化为“特征空间里的线性模型（对参数/特征线性）”，并且计算只需要内积（Gram 矩阵），不需要显式写出特征映射 φ(x)。
---
# 核方法（Kernel Methods）与“核函数判定链路”

## 1. 一句话
- 核方法把“输入空间里看起来很非线性的几何/决策边界”，转化为“特征空间里的线性模型（对参数/特征线性）”，并且计算只需要内积（Gram 矩阵），不需要显式写出特征映射 `φ(x)`。

## 1.1 这里的“线性 / 非线性”到底指什么
- **线性**：通常指 `f(x)=<w,φ(x)>+b` 这种“对 `w`（或对系数）线性”的模型形式；或者说“在特征空间里是超平面”。
- **非线性**：通常指 `x -> f(x)` 在输入空间里表现为非线性（因为 `φ`/`k` 非线性），从而得到非线性的决策边界。
- 这和“是否可逆/双射”不是一回事；并且多输入函数要说清“对哪个变量线性”。见 `math/线性代数/modules/Linearity.md`
